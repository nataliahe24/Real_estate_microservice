package com.powerup.realestate.commons.configurations.utils;

public final class Constants {
    public static final String SAVE_LOCATION_RESPONSE_MESSAGE = "Ubicacion creada exitosamente." ;
    public static final String PAGEABLE_FIELD_CITY = "city";
    public static final String PAGEABLE_FIELD_DEPARTMENT = "department";

    private Constants() {
        throw new IllegalStateException("Utility class");
    }

    public static final String SAVE_CATEGORY_RESPONSE_MESSAGE = "Categoria creada exitosamente";
    public static final String PAGEABLE_FIELD_NAME = "name";
}
